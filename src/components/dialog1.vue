<template>
    ...
    <div class="container">
        <div class="header">
        <el-header>
            <span style="float: left; "><img  id='iconImage' :src=iconImageSrc></span>
            <span  id='title' style="float: left; padding:  0 10px ">{{title}}</span>
            <el-button  icon="el-icon-close" type="primary" circle onclick=window.close()></el-button>
        </el-header>
        </div>
        <div class="main">
            <div class="image">
                <img id="contentImage" :src="contentImageUrl" />
            </div>
        </div>
...
<script>
    ...
    import contentImageUrl from '../img/content.jpg';
    export default {
        name:'dialog1',
        data() {
            return {
